...
from src.simulator.rl_agent import QAgent, SarsaAgent
...
def load_data(data_dir):
    ...
    travel_time_path = os.path.join(data_dir, 'travel_time_matrix.npy')
    travel_time_matrix = np.load(travel_time_path)
    
    # Use travel time as distance for simplicity
    distance_matrix = travel_time_matrix.copy()
    ...
    return graph, distance_matrix, travel_time_matrix, base_locations, hospital_node, call_data_path
...
def train_rl(
    ...
    Args:
        ...
        travel_time_matrix: Pre-computed travel time matrix
    ...
    if data_dir is not None:
        ...
        # Use provided data
        if graph is None or travel_time_matrix is None or base_locations is None or hospital_node is None:
            raise ValueError("Must provide either data_dir or all required data (graph, travel_time_matrix, base_locations, hospital_node)")
        distance_matrix = travel_time_matrix  # Use travel time as proxy for distance
        
    # Create environment
    env = AmbulanceEnv(
        graph=graph,
        call_data_path=call_data_path if data_dir else None,
        distance_matrix=distance_matrix,
        travel_time_matrix=travel_time_matrix,
        num_ambulances=1,  # Start with 1 ambulance for testing
        base_locations=base_locations[:1],  # Use only first base location
